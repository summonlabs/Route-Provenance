...
int main() {
  std::cout << "example=adaptive_decision version=" << kVersionString << "\n";
  Example example;
  DependencyNotification policy;
  policy.kind = DependencyKind::PolicyAdvance;
  policy.policy_generation = PolicyGeneration::from_value(3);
  policy.authority = example.context();
  static_cast<void>(example.store.notify_dependency(policy));
  DependencyNotification evidence;
  evidence.kind = DependencyKind::EvidenceAdvance;
  evidence.evidence_generation = EvidenceGeneration::from_value(9);
  evidence.authority = example.context();
  static_cast<void>(example.store.notify_dependency(evidence));

  Bindings bindings;
  AdaptationBinding adaptation;
  adaptation.decision = AdaptationDecisionId::from_value(12);
  adaptation.generation = AdaptationGeneration::from_value(4);
  adaptation.policy = PolicyGeneration::from_value(3);
  adaptation.evidence = EvidenceGeneration::from_value(9);
  adaptation.decision_digest = Digest::hash("adaptation-decision:12");
  bindings.adaptation = adaptation;

  const Result<Publication> published =
      example.publish(104, "example/adaptive", 1, ReasonCode::AdaptiveChange,
                      RootReason::InitialPublication, {}, bindings);
  if (report("publish_adaptive_change", published.outcome()) != 0) {
    return 1;
  }
  std::cout << "adaptation_decision=12 generation=4 policy=3 evidence=9\n";
  ExplainRequest request;
  request.subject = published.value().node;
  request.mode = ExplainMode::PolicyOnly;
  const Result<Explanation> explanation = example.store.explain(request);
  if (!explanation.has_value()) {
    return 1;
  }
  std::cout << "explanation_steps=" << explanation.value().steps.size()
